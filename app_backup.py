...
# --- LOAD CLEANED DATA FROM DATA PIPELINE ---
@st.cache_data
def load_production_dataset():
    """
    Load cleaned dataset from data_pipeline.py export
    This ensures consistency between EDA and Streamlit app
    """
    import os
    
    # Song mapping by genre
    song_manifest = {
    ...
    # Try to load cleaned CSV from data pipeline
    csv_path = 'data/cleaned_movies_data.csv'
    
    if os.path.exists(csv_path):
        df = pd.read_csv(csv_path)
        st.sidebar.info("✓ Using cleaned data from data_pipeline.py")
        
        # Add Recommended_Song column if it doesn't exist
        if 'Recommended_Song' not in df.columns:
            df['Recommended_Song'] = df['Genre'].map(lambda g: song_manifest.get(g, ["Unknown Song"])[0])
        
        # Rename Weather to Associated_Weather for consistency
        if 'Weather' in df.columns and 'Associated_Weather' not in df.columns:
            df.rename(columns={'Weather': 'Associated_Weather'}, inplace=True)
    else:
        # Fallback: Create sample data if pipeline hasn't been run
        st.sidebar.warning("⚠ Pipeline data not found. Using sample data.")
...
if st.session_state.show_all_movies:
    ...
    with filter_col1:
        genre_filter = st.multiselect("Filter by Genre:", df['Genre'].unique(), default=df['Genre'].unique(), key="movie_genre")
    
    with filter_col2:
        rating_filter = st.slider("Minimum Rating:", 0.0, 10.0, 0.0, key="movie_rating")
...
if st.session_state.show_playlist:
    ...
    with playlist_col1:
        selected_genres = st.multiselect("Select genres for playlist:", df['Genre'].unique(), key="playlist_genre")
        min_rating = st.slider("Minimum song rating:", 0.0, 10.0, 5.0, key="playlist_rating")
